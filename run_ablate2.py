"""Ablation batch 2: multi-annotator multi-head training + denser sampling."""
import json
import time
...
import models
import train as T
from run_ablate import predict
from run_annot import eval_probs

OUT = Path(__file__).parent / "results"
CONFIGS = [
    ("multih", dict(arch="multih", norm="patient", tag="MULTI", mixup=0.2,
                    chanind=False, epochs=45, stride=None)),
    ("multih_s8", dict(arch="multih", norm="patient", tag="MULTI", mixup=0.2,
                       chanind=False, epochs=35, stride=8)),
]
...
def main():
    ...
    for name, cfg in CONFIGS:
        ...
        model, best_val, _ = T.train_model(
            ...
            stride=cfg["stride"], patience=12, lr=1.2e-3,
